-- Fix purchase_order_items branch_id mapping (v2)
-- This script correctly maps branch_id based on order_number instead of order_id
-- Handles duplicate items by keeping only unique combinations
...
-- Using ROW_NUMBER to handle duplicates
...
FROM (
  SELECT 
    ...
    item,
    ROW_NUMBER() OVER (
      PARTITION BY po.order_number, po.branch_id, item->>'itemId'
      ORDER BY po.id DESC
    ) as rn
