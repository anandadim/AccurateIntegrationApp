<template>
  <div class="sync-manager">
    <h2>💵 Sales Receipt Sync Manager</h2>
    ...
    <div class="card" v-if="selectedBranch">
      ...
      <div v-if="checkResult" class="result-box">
        ...
        <div class="sync-recommendation">
          <p v-if="checkResult.summary.needSync>0" class="need-sync">⚠️ Need to sync <strong>{{ checkResult.summary.needSync }}</strong> receipts</p>
    ...
    <div class="card" v-if="selectedBranch">
      ...
      <button @click="syncReceipts" :disabled="syncing || !checkResult || checkResult.summary.needSync===0" class="btn btn-success">
  ...
  setup(props){
    ...
    const checkSync=async()=>{
      ...
      checking.value=true;error.value='';checkResult.value=null;try{
        const res=await apiService.checkReceiptSyncStatus({branchId:selectedBranch.value,dateFrom:dateFrom.value,dateTo:dateTo.value,dateFilterType:dateFilterType.value});
    ...
    const syncReceipts=async()=>{
      ...
      try{
        const res=await apiService.syncReceipts({branchId:selectedBranch.value,dateFrom:dateFrom.value,dateTo:dateTo.value,dateFilterType:dateFilterType.value,batchSize:batchSize.value,batchDelay:batchDelay.value});
    ...
    return{branches:props.branches,selectedBranch,dateFrom,dateTo,dateFilterType,batchSize,batchDelay,checking,syncing,reloading,checkResult,syncResult,syncProgress,error,onBranchChange,reloadBranches,checkSync,syncReceipts}
  }
}
</script>

<style scoped>
/* Import styles from SyncManager */
.sync-manager {
  max-width: 900px;
  margin: 0 auto;
  padding: 20px;
