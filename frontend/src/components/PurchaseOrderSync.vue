<template>
  <div class="sync-manager">
    <h2>📝 Purchase Order Sync Manager</h2>
    ...
    <div class="card" v-if="selectedBranch">
      <h3>2. Cek Jumlah Purchase Order</h3>
      ...
      <div v-if="checkResult" class="result-box">
        ...
        <div class="sync-recommendation">
          <p v-if="checkResult.summary.needSync>0" class="need-sync">⚠️ Need to sync <strong>{{ checkResult.summary.needSync }}</strong> orders</p>
    ...
    <div class="card" v-if="selectedBranch">
      ...
      <div class="form-group"><label>Delay (ms)</label><input type="number" v-model.number="batchDelay" min="100" max="2000" step="100" class="number-input"/></div>
      <button @click="syncOrders" :disabled="syncing || !checkResult || checkResult.summary.needSync===0" class="btn btn-success">
...
export default {
  ...
  setup(props){
    ...
    const checkSync=async()=>{
      ...
      checking.value=true;error.value='';checkResult.value=null
      try{
        const res=await apiService.checkPurchaseOrderSyncStatus({branchId:selectedBranch.value,dateFrom:dateFrom.value,dateTo:dateTo.value,dateFilterType:dateFilterType.value})
    ...
    const syncOrders=async()=>{
      ...
      try{
        const res=await apiService.syncPurchaseOrders({branchId:selectedBranch.value,dateFrom:dateFrom.value,dateTo:dateTo.value,dateFilterType:dateFilterType.value,batchSize:batchSize.value,batchDelay:batchDelay.value})
    ...
    return{branches:props.branches,selectedBranch,dateFrom,dateTo,dateFilterType,batchSize,batchDelay,checking,syncing,checkResult,syncResult,syncProgress,error,onBranchChange,checkSync,syncOrders}
  }
}
</script>
...
.card h3 {margin:0 0 16px;color:#333;font-size:18px;}
.branch-selector {display:flex;gap:12px;align-items:center;}
.select-input,.date-input,.number-input{padding:8px 12px;border:1px solid #ddd;border-radius:4px;font-size:14px;}
.btn{padding:8px 16px;border:none;border-radius:4px;cursor:pointer;font-size:14px;transition:all .2s;}
...
.result-box{margin-top:20px;padding:16px;border-radius:4px;background:#f8f9fa;border:1px solid #e9ecef;}
.result-box.success{background:#d4edda;border-color:#c3e6cb;color:#155724;}
.result-box h4{margin:0 0 12px;color:#333;}
.sync-status-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:16px;}
.status-card{display:flex;align-items:center;padding:12px;border-radius:4px;background:white;border:1px solid #e9ecef;}
.status-icon{font-size:24px;margin-right:8px;}
.status-content{flex:1;}
.status-label{font-size:12px;color:#666;text-transform:uppercase;}
.status-value{font-size:18px;font-weight:bold;color:#333;}
.status-card.new .status-value{color:#28a745;}
.status-card.updated .status-value{color:#ffc107;}
.status-card.unchanged .status-value{color:#6c757d;}
.status-card.total .status-value{color:#007bff;}
.sync-recommendation{padding:12px;border-radius:4px;margin-bottom:16px;}
.sync-recommendation .need-sync{background:#fff3cd;border:1px solid #ffc107;padding:12px;border-radius:4px;color:#856404;margin:0;}
.sync-recommendation .up-to-date{background:#d4edda;border:1px solid #28a745;padding:12px;border-radius:4px;color:#155724;margin:0;}
.progress-box{margin-top:16px;padding:16px;background:#e9ecef;border-radius:4px;}
.progress-header{display:flex;justify-content:space-between;margin-bottom:8px;font-size:14px;color:#666;}
.progress-bar{height:8px;background:#dee2e6;border-radius:4px;overflow:hidden;}
.progress-fill{height:100%;background:#007bff;transition:width .3s ease;}
...
.result-item .label{font-weight:500;color:#666;}
.result-item .value{font-weight:bold;color:#333;}
.success-text{color:#28a745;}.error-text{color:#dc3545;}
.error-box{margin-top:20px;padding:16px;border-radius:4px;background:#f8d7da;border:1px solid #f5c6cb;color:#721c24;}
